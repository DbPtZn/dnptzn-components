...
import { FractalContainerConfig, containerEnum } from '..'
...
onMounted(() => {
  ...
  /** 自检测：移除空容器 */
  if (child.value.type === containerEnum.SIMPLE_CONTAINER && child.value.children.length === 0) {
    console.log('移除空容器')
  ...
  /** 
    ...
    if (child.value.name === containerEnum.SIMPLE_CONTAINER && child.value.children.length === 1) {
